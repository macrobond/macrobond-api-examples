...
def poll_for_updates(
    api: WebApi, subscription_list: SubscriptionList, dbmock: dict[str, Series]
) -> None:
    # Poll for updates
    while True:
...
def set_and_poll() -> None:
    ...
    with WebClient() as api:
        ...
        for name, series in dbmock.items():
            print(
                f'Store series "{name}" in DB, last_modified "{series.last_modified}"'
            )
    ...
    with WebClient() as api:
        ...
        for series in api.get_many_series(new_series):
            if not series.is_error:  # Error handling omitted
                dbmock[series.primary_name] = series
                print(
                    f'Store series "{series.primary_name}" in DB, last_modified "{series.last_modified}"'
                )
